...
// @homepageURL  https://aajkrvv.github.io/web-digital-wellbeing-script/
...
(function () {
  ...
  function getDomain() {
    return location.hostname.replace(/^www\./, "") || "unknown";
  }
  function readSec(date, dom) {
    return parseInt(GM_getValue(KEY_PREFIX + date + "_" + dom, "0"), 10) || 0;
  }
  function writeSec(date, dom, s) {
    GM_setValue(KEY_PREFIX + date + "_" + dom, String(s));
  }
  function loadAll() {
    const r = {};
    try {
    ...
    try {
      ...
      GM_listValues().forEach((k) => {
        if (
          ...
          k.slice(KEY_PREFIX.length, KEY_PREFIX.length + 10) < c
        )
          GM_deleteValue(k);
      });
    } catch (e) {}
  ...
  function enqueue(fn) {
    ...
    try {
      ...
      if (_q.length) {
        const nx = _q.shift();
        Promise.resolve().then(() => enqueue(nx.fn).then(nx.r));
      }
    }
  ...
  function getRules() {
    try {
      return JSON.parse(GM_getValue("wbt_rules", "[]"));
    } catch {
      return [];
    }
  }
  function saveRules(r) {
    GM_setValue("wbt_rules", JSON.stringify(r));
  }
  function getGroups() {
    try {
      return JSON.parse(GM_getValue("wbt_groups", "[]"));
    } catch {
      return [];
    }
  }
  function saveGroups(g) {
    GM_setValue("wbt_groups", JSON.stringify(g));
  }
  ...
  function findRule(domain) {
    ...
    return (
      getRules().find((r) => {
        ...
        if (r.days && r.days.length > 0 && !r.days.includes(today))
          return false;
        return true;
      }) || null
    );
  ...
  function getGroupUsage(groupId) {
    ...
    return getRules()
      ...
      .reduce((s, r) => s + readSec(td, r.domain), 0);
  }
  ...
  function showLockScreen(domain, rule, isGroup) {
    ...
    el.id = "wbt-ov";
    const descGroup = isGroup
      ? "그룹 사용 시간 제한에 도달했습니다."
      : "오늘의 사용 시간 제한에 도달했습니다.";
    el.innerHTML = `
            <div class="wbt-ob">
  ...
  function showDelayScreen(domain, rule, onPass) {
    ...
    _injectOvStyle();
    const freeLbl =
      rule.freeMins > 0 ? `${rule.freeMins}분동안 잠금 해제` : "잠금 해제";
    ...
    const onFocus = () => {
      if (openBtn.classList.contains("off")) {
        resetCountdown();
        tick();
      }
    };
  ...
  function initLockMonitor(domain, rule) {
    ...
    const grp = rule.groupId
      ? getGroups().find((g) => g.id === rule.groupId)
      : null;
    ...
    setInterval(() => {
      ...
      if (
        hasIndivLim &&
        readSec(getLocalDate(), domain) >= rule.limitMins * 60
      ) {
        showLockScreen(domain, rule, false);
        return;
      }
  ...
  function checkAndShowOverlay() {
    ...
    if (!isLocked && rule.groupId) {
      ...
      if (
        grp?.limitMins > 0 &&
        getGroupUsage(rule.groupId) >= grp.limitMins * 60
      )
        isLocked = true;
    }
    if (isLocked) {
      if (!document.getElementById("wbt-ov"))
        showLockScreen(domain, rule, !!rule.groupId);
      return;
    }
    ...
    if (rule.delaySec > 0) {
      ...
      if (!document.getElementById("wbt-ov")) {
        showDelayScreen(domain, rule, () => {
          const exp =
            Date.now() + (rule.freeMins > 0 ? rule.freeMins * 60000 : 86400000);
  ...
  function initTracker() {
    ...
    const active = () =>
      !document.hidden &&
      document.hasFocus() &&
      !document.getElementById("wbt-ov");
    setInterval(() => {
      const n = Date.now();
      if (active()) {
        const e = Math.min(Math.round((n - lt) / 1000), MAX_TICK_SEC);
        if (e > 0) acc += e;
      }
      lt = n;
    }, 1000);
    ...
    window.addEventListener("beforeunload", () => {
      if (acc < 1) return;
      const td = getLocalDate();
      writeSec(td, dom, readSec(td, dom) + acc);
      acc = 0;
    });
  ...
  function initTapFeedback(appEl) {
    ...
    const clr = () =>
      setTimeout(() => {
        appEl
          .querySelectorAll(".tap-sc,.tap-dm")
          .forEach((el) => el.classList.remove("tap-sc", "tap-dm"));
      }, 80);
  ...
  function iconLI(domain, idx) {
    const c = PAL[idx % PAL.length],
      i = domain.replace(/\..+$/, "").charAt(0).toUpperCase();
  ...
  function renderTimerList(D, td) {
    ...
    return rules
      .map((r) => {
        const activeToday =
          !r.days || r.days.length === 0 || r.days.includes(todayDow);
        const used = (D[td] || {})[r.domain] || 0;
        const lim = r.limitMins || 0;
        const pct =
          lim > 0 ? Math.min(Math.round((used / (lim * 60)) * 100), 100) : 0;
        ...
        const statusTxt = !activeToday
          ? "오늘은 타이머 꺼짐"
          : over
            ? "오늘 제한 초과"
            : lim > 0
              ? fmtRemain(rem)
              : `${Math.floor(used / 60)}분 사용`;
  ...
  function renderMain() {
    ...
    const top3 = sorted.slice(0, 3),
      rest = sorted.slice(3).reduce((s, [, v]) => s + v, 0);
    const COLS = ["#4285F4", "#00BCD4", "#34A853"];
    const dSegs = [
      ...top3.map(([, v], i) => ({ v, c: COLS[i] })),
      ...(rest > 0 ? [{ v: rest, c: "#C7C7CC" }] : []),
    ];
    const w7 = Array.from({ length: 7 }, (_, i) => getLocalDate(6 - i));
    const wTot = w7.reduce(
      (s, d) => s + Object.values(D[d] || {}).reduce((a, b) => a + b, 0),
      0,
    );
    const wAct = w7.filter(
      (d) => Object.values(D[d] || {}).reduce((a, b) => a + b, 0) > 0,
    ).length;
    const wAvg = wAct ? Math.round(wTot / wAct) : 0;
    const wS = {};
    for (const d of w7)
      for (const [s, v] of Object.entries(D[d] || {})) wS[s] = (wS[s] || 0) + v;
    const wTop3 = Object.entries(wS)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3);
    return `
        <div class="hdr">
  ...
  function renderDash() {
    ...
    const tots = days.map((d) =>
      Object.values(D[d] || {}).reduce((a, b) => a + b, 0),
    );
    const maxT = Math.max(...tots, 1);
    const allS = {};
    for (const d of days)
      for (const [s, v] of Object.entries(D[d] || {}))
        allS[s] = (allS[s] || 0) + v;
    const top3 = Object.entries(allS)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([s]) => s);
    ...
    const bc = (s) => {
      ...
      return i >= 0 ? C3[i] : "#C7C7CC";
    };
    const selD = D[_dd] || {},
      selT = Object.values(selD).reduce((a, b) => a + b, 0);
    const selS = Object.entries(selD).sort((a, b) => b[1] - a[1]);
    const sdt = new Date(_dd + "T00:00:00");
    const sLbl =
      _dd === td
        ? "오늘"
        : `${sdt.getMonth() + 1}월 ${sdt.getDate()}일 (${DOW_KOR[sdt.getDay()]})`;
    return `
        <div class="hdr">
                ...
                ${days
                  .map((d, i) => {
                    const dD = D[d] || {},
                      dT = tots[i];
                    const bH =
                      dT > 0 ? Math.max(Math.round((dT / maxT) * 90), 4) : 0;
                    const isSel = d === _dd,
                      isT = d === td;
                    const dt = new Date(d + "T00:00:00");
                    const t3s = top3
                      .map((s) => [s, dD[s] || 0])
                      .filter(([, v]) => v > 0);
                    const rv = Object.entries(dD)
                      .filter(([s]) => !top3.includes(s))
                      .reduce((s, [, v]) => s + v, 0);
                    const segs = [...t3s, ...(rv > 0 ? [["__", rv]] : [])];
                    const segH = segs
                      .map(
                        ([s, v]) =>
                          `<div style="flex:${v};background:${s === "__" ? "#C7C7CC" : bc(s)}"></div>`,
                      )
                      .join("");
  ...
  function renderWeekly() {
    ...
    for (const d of wd)
      for (const [s, v] of Object.entries(D[d] || {})) wS[s] = (wS[s] || 0) + v;
    const wTop5 = Object.entries(wS)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const wTop3Sites = wTop5.slice(0, 3).map(([s]) => s);
    const C3 = ["#4285F4", "#00BCD4", "#34A853"];
    const bc = (s) => {
      const i = wTop3Sites.indexOf(s);
      return i >= 0 ? C3[i] : "#C7C7CC";
    };
    const ws = new Date(wd[0] + "T00:00:00"),
      we = new Date(wd[6] + "T00:00:00");
    ...
    return `
        ...
        <div class="card" style="padding:20px 18px 16px">
            ...
            <div class="wk-chart-outer">
                ${
                  showAvg
                    ? `<div class="wk-avg-line" style="bottom:calc(20px + ${avgPx}px)">
                ...
                <div class="wk-bars">
                    ${wd
                      .map((d, i) => {
                        ...
                        const pct =
                          dT[i] > 0
                            ? Math.max(Math.round((dT[i] / mxT) * 100), 4)
                            : 0;
                        const isT = d === td;
                        const t3s = wTop3Sites
                          .map((s) => [s, dD[s] || 0])
                          .filter(([, v]) => v > 0);
                        const rv = Object.entries(dD)
                          .filter(([s]) => !wTop3Sites.includes(s))
                          .reduce((s, [, v]) => s + v, 0);
                        const segs = [...t3s, ...(rv > 0 ? [["__", rv]] : [])];
                        const segH = segs
                          .map(
        ...
        <div class="card">
            ${rules
              .map((r) => {
                ...
                const pct =
                  lim > 0
                    ? Math.min(Math.round((used / (lim * 60)) * 100), 100)
                    : 0;
                const rem = lim > 0 ? Math.max(lim * 60 - used, 0) : -1;
                const over = lim > 0 && used >= lim * 60;
                const bc = over
        ...
        <div class="card">
            ${groups
              .map((g) => {
                ...
                const grpPct =
                  grpLim > 0
                    ? Math.min(Math.round((grpUsed / (grpLim * 60)) * 100), 100)
                    : 0;
    ...
    return `
        ...
        <div class="form-card">
            ...
            <div class="day-row">
                ${DOW_KOR.map(
                  (d, i) => `
                    <button class="day-btn${days.includes(i) ? " on" : ""}" type="button" data-day="${i}">${d}</button>`,
                ).join("")}
            </div>
        </div>
  ...
  function renderGroupForm() {
    ...
    const g = isEdit
      ? getGroups().find((x) => x.id === _editGroupId) || {}
      : {};
    const allRules = getRules();
    const memberIds = allRules
      .filter((r) => r.groupId === _editGroupId)
      .map((r) => r.id);
    return `
        <div class="hdr">
            <button class="ico ico-back" id="btn-back" type="button">${ICO.back}</button>
  ...
  function attachEv(view) {
    ...
    if (view === "dash") {
      ...
      if (sc) {
        ...
        sc.querySelector(".dpill.sel")?.scrollIntoView({
          behavior: "auto",
          inline: "center",
          block: "nearest",
        });
      ...
      document.getElementById("schart")?.addEventListener("click", (e) => {
        const c = e.target.closest(".scol");
        if (c) nav("dash", { date: c.dataset.date });
      });
    }
    ...
    if (view === "weekly") {
      ...
      document.querySelectorAll(".wk-tab").forEach((b) => {
        b.addEventListener("click", () =>
          nav("weekly", { wo: parseInt(b.dataset.wo) }),
        );
      });
    }
    ...
    if (view === "rules") {
      ...
      document.querySelectorAll(".edit-btn[data-rule-id]").forEach((b) => {
        b.addEventListener("click", () =>
          nav("rule-form", { ruleId: b.dataset.ruleId }),
        );
      });
      document.querySelectorAll(".edit-btn[data-group-id]").forEach((b) => {
        b.addEventListener("click", () =>
          nav("group-form", { groupId: b.dataset.groupId }),
        );
      });
    }
    ...
    if (view === "rule-form") {
      ...
      const days = new Set(
        _editRuleId
          ? getRules().find((r) => r.id === _editRuleId)?.days || []
          : [],
      );
      ...
      const notify = {
        min1: !!(
          _editRuleId &&
          getRules().find((r) => r.id === _editRuleId)?.notify?.min1
        ),
        min5: !!(
          _editRuleId &&
          getRules().find((r) => r.id === _editRuleId)?.notify?.min5
        ),
        min10: !!(
          _editRuleId &&
          getRules().find((r) => r.id === _editRuleId)?.notify?.min10
        ),
      };
      ...
      on("btn-save-rule", () => {
        ...
        const delaySec = Math.max(
          0,
          parseInt(document.getElementById("r-delay")?.value || "0", 10) || 0,
        );
        const freeMins = Math.max(
          0,
          parseInt(document.getElementById("r-free")?.value || "0", 10) || 0,
        );
        const limitMins = Math.max(
          0,
          parseInt(document.getElementById("r-limit")?.value || "0", 10) || 0,
        );
    ...
    if (view === "group-form") {
      ...
      on("btn-save-group", () => {
        ...
        const limitMins = Math.max(
          0,
          parseInt(document.getElementById("g-limit")?.value || "0", 10) || 0,
        );
        if (!name) {
          alert("그룹 이름을 입력해주세요");
          return;
        ...
        if (gid) {
          ...
          gid = genId();
          groups.push({ id: gid, name, limitMins });
        }
        saveGroups(groups);
        /* 멤버 업데이트 */
        const rules = getRules().map((r) => {
